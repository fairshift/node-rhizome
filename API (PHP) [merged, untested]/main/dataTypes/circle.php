...
// Unfunctionable at this time
/*

	- $block and $response are two different patterns (need to update)
	- code is missing in
...
function getCommoners(){
    ...
    if(!$response = existingCache($db, $cache)){
      ...
      while($row = mysqli_fetch_array($result, MYSQLI_ASSOC) && isAvailable($row)){
  		...
  		$block = mergeBlocks('getCommoners', $block, $buffer);
